...
# 2️⃣ 사업명 선택 및 입력
if 사업리스트[기관]:
    사업명_선택 = st.selectbox("공고/사업명을 선택하세요:", 사업리스트[기관] + ['기타 (직접 입력)'], help="해당 부서의 기존 사업 중에서 선택하거나 기타를 선택해 새로 입력할 수 있습니다.")
    if 사업명_선택 == '기타 (직접 입력)':
...
# 3️⃣ 일반 정보 입력
정부24등록 = st.radio("정부24에 등록되어 있습니까?", ['예', '아니오'], help="정부24 사이트에 이미 등록되어 있는 사업인지 여부를 선택합니다.")

가구 = st.text_input("가구", help="지원대상이 되는 가구 유형 (예: 다자녀가구, 한부모가구 등)을 입력하세요.")
# ✅ 나이 부분: 선택형 + 직접 입력 병행
나이옵션 = ['0~5세', '6~7세', '8~10세', '10세 이하', '미취학 아동', '초등학생', '중학생', '고등학생', '성인', '전체', '기타 (직접 입력)']
나이선택 = st.selectbox("대상 연령대", 나이옵션, help="지원대상자의 연령대 범주를 선택하세요.")

if 나이선택 == '기타 (직접 입력)':
    나이직접입력 = st.text_input("연령대 직접 입력", help="자유롭게 연령 범위를 작성하세요 (예: 3~9세, 출생~만 5세 등)")
    나이최종 = 나이직접입력
else:
    나이최종 = 나이선택

접수기관명 = st.text_input("접수기관명", help="공고를 담당하는 접수기관명을 입력하세요.")
서비스명 = st.text_input("서비스명", help="해당 지원 서비스의 이름을 작성하세요.")
지원대상 = st.text_area("지원대상", help="지원대상에 해당하는 구체적인 인원, 조건을 작성하세요.")
지원내용 = st.text_area("지원내용", help="지원하는 내용(금액, 물품, 서비스 등)을 상세히 작성하세요.")
...
# 5️⃣ 관리자 전용 영역
with st.expander("📊 [관리자 전용] 수집된 데이터 보기"):
    admin_password = st.text_input("관리자 비밀번호 입력", type="password")
    if admin_password == "admin1234":  # 🔑 관리자 비밀번호 설정 (변경 가능)
